        ...
        <div className="container mx-auto mt-10">
          <div className="flex shadow-md my-10">
            <div className="w-3/4 bg-white px-10 py-10">
                  ...
                  <div key={item.id} >
                    <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                      <div className="flex w-2/5">
                        <div className="w-20">
                          <img className="h-24" src={item.productImage} alt="" />
                        </div>
                        <div className="flex flex-col justify-between ml-4 flex-grow">
                          <span className="font-bold text-sm">{item.bookname}</span>
                          <span className="text-red-500 text-xs">{item.brand}</span>
                          <button className='className="font-semibold hover:text-red-500 text-gray-500 text-xs"' onClick={()=>handleRemove(item._id)}>Remove</button>
                        </div>
                      </div>
                      <div className="flex justify-center w-1/5">
                        <svg onClick={()=>handleDecress(item)} className="fill-current text-gray-600 w-3" viewBox="0 0 448 512"><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" />
                        </svg>

                        <input className="mx-2 border text-center w-8" type="text" value={item.quantity} />

                        <svg onClick={()=>handleIncress(item)} className="fill-current text-gray-600 w-3" viewBox="0 0 448 512">
              ...
              <Link to="/books" className="flex font-semibold text-indigo-600 text-sm mt-10">
